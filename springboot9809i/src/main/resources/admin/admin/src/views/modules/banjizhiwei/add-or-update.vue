	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="班级职位" prop="banjizhiwei">
					<el-select :disabled="ro.banjizhiwei" v-model="ruleForm.banjizhiwei" placeholder="请选择班级职位" >
						<el-option
							v-for="(item,index) in banjizhiweiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="班级职位" prop="banjizhiwei">
					<el-input v-model="ruleForm.banjizhiwei"
						placeholder="班级职位" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="任命日期" prop="renmingriqi">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.renmingriqi" 
						type="date"
						:readonly="ro.renmingriqi"
						placeholder="任命日期"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.renmingriqi" label="任命日期" prop="renmingriqi">
					<el-input v-model="ruleForm.renmingriqi" placeholder="任命日期" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="任命内容" prop="renmingneirong">
					<el-input v-model="ruleForm.renmingneirong" placeholder="任命内容" clearable  :readonly="ro.renmingneirong"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="任命内容" prop="renmingneirong">
					<el-input v-model="ruleForm.renmingneirong" placeholder="任命内容" readonly></el-input>
				</el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				banjizhiwei: [
					{ required: true, message: '班级职位不能为空', trigger: 'blur' },
				],
				jiaoshigonghao: [
	...
	created() {
		this.ruleForm.renmingriqi = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='banjizhiwei'){
							this.ruleForm.banjizhiwei = obj[o];
							this.ro.banjizhiwei = true;
							continue;
						}
						if(o=='jiaoshigonghao'){
						...
						if(o=='renmingriqi'){
							this.ruleForm.renmingriqi = obj[o];
							this.ro.renmingriqi = true;
							continue;
						}
						if(o=='renmingneirong'){
							this.ruleForm.renmingneirong = obj[o];
							this.ro.renmingneirong = true;
							continue;
						}
						if(o=='xuehao'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `banjizhiwei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.banjizhiweiCrossAddOrUpdateFlag = false;
						 this.parent.search();
